...
export function presentMitigationRun(run: RerouteBundle): RunPresentation {
  ...
  const reverting = ["recovery_scheduled", "recovery_claimed", "recovery_running"].includes(lifecycle) || execution === "compensating";
  ...
  else if (execution === "compensation_blocked" || lifecycle === "recovery_blocked" || unknown > 0) { label = "Needs attention"; detail = unknown > 0 ? `${unknown} effect${unknown === 1 ? " is" : "s are"} unknown; ${remaining} changes may remain.` : `${remaining} router change${remaining === 1 ? "" : "s"} remain.`; tone = "bad"; }
...
export function recentRunAsBundle(run: Partial<RerouteBundle> & { bundle_id: number; state: string; created_at: string }): RerouteBundle {
