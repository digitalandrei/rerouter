            ...
            >
              ...
              <TopicTable>
                ...
                <tbody>
                  ...
                  <tr><td className={tdClass}>BGP advertisement</td><td className={tdClass}>Add/remove a prefix-list permit for one peer at an explicit sequence number, then soft-clear outbound. The sequence is chosen from a read of that list taken in the same SSH session that writes it, so the entry lands before the list&rsquo;s terminating deny instead of after it (where IOS auto-numbering would put it, and where it would never be reached). If the gap holds no free sequence the action fails closed and asks you to renumber the list.</td><td className={tdClass}>Remove the exact entry that was added, by its recorded sequence</td></tr>
