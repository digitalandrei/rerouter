...
it("does not reopen a closed run when a polling response arrives late", async () => {
  ...
  const user = userEvent.setup(); render(<AuthProvider><MemoryRouter><ActiveRunsTab /></MemoryRouter></AuthProvider>);
  await user.click(await screen.findByRole("button", { name: "View run" })); const closeButtons = await screen.findAllByRole("button", { name: "Close" }); await user.click(closeButtons.find((button) => button.textContent === "Close")!);
