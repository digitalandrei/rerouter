...
export function ActiveRunsTab() {
  ...
  const [dismissRecoveryOpen, setDismissRecoveryOpen] = useState(false);
  ...
  const dismissibleRecovery = selected?.latest_recovery?.id === displayedRecoveryId && selected.latest_recovery.dismiss?.available ? selected.latest_recovery.dismiss : null;
  ...
  return <>
    ...
    {selected && <Card><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div className="space-y-2"><h2 className="font-semibold leading-none">Mitigation run #{selected.id}</h2><CardDescription>Execution outcome and current router changes are separate. Revert always targets this explicitly selected run.</CardDescription></div><Button variant="outline" onClick={closeSelectedRun} disabled={busy}>Back to active runs</Button></div></CardHeader><CardContent className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">{canAct && dismissibleRecovery && <Button variant="outline" onClick={() => setDismissRecoveryOpen(true)} disabled={busy}>Dismiss failed revert</Button>}{canAct && selected.take_control?.available && <Button variant="outline" onClick={() => void takeControl()} disabled={busy} loading={pendingAction === "take_control"} loadingLabel="Cancelling automatic recovery…">Cancel automatic recovery</Button>}{canAct && !preview && <Button variant="outline" onClick={() => void previewRevert()} disabled={busy || selected.revert?.available === false} loading={pendingAction === "preview_revert"} loadingLabel="Preparing revert preview…">Preview revert</Button>}{canAct && !preview && <Button variant="destructive" onClick={() => void revertNow()} disabled={busy || selected.revert?.available === false} loading={pendingAction === "direct_revert"} loadingLabel={directRevertStage === "starting" ? "Starting revert…" : "Calculating exact revert…"}>Revert now</Button>}{canAct && preview && <Button variant="destructive" onClick={() => void confirmRevert()} disabled={busy || !preview.plan_id || !preview.preview_token} loading={pendingAction === "confirm_revert"} loadingLabel="Starting revert…">Apply reviewed revert</Button>}</div>
    </CardContent></Card>}
    {selected && displayedRecoveryId && dismissibleRecovery && <ConfirmDialog open={dismissRecoveryOpen} onOpenChange={setDismissRecoveryOpen} title={`Dismiss failed revert #${displayedRecoveryId}?`} description="This sends no router commands. The failed no-write recovery remains in history and the original mitigation changes remain active." confirmLabel="Dismiss failed revert" requireText={dismissibleRecovery.confirmation_phrase ?? `DISMISS RECOVERY #${displayedRecoveryId}`} onConfirm={async () => { await api.bundles.dismissRecovery(displayedRecoveryId, dismissibleRecovery.confirmation_phrase ?? `DISMISS RECOVERY #${displayedRecoveryId}`); setDismissRecoveryOpen(false); recoveryIdRef.current=null; recoveryRunRef.current=null; setRecoveryRun(null); toast.success(`Recovery #${displayedRecoveryId} dismissed; original changes remain active.`); await load(true); }} />}
  </>;
}
