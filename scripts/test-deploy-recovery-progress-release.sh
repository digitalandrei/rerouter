...
script="$(dirname "$0")/deploy-recovery-progress-release.sh"
grep -Fq "id=1 AND parent_bundle_id IS NULL" "$script"
grep -Fq "id=2 AND parent_bundle_id=1" "$script"
grep -Fq "SELECT COUNT(*) FROM reroutes inverse JOIN reroutes original" "$script"
grep -Fq "SELECT COUNT(*) FROM device_change_windows" "$script"
...
if grep -Eq '/api/(manual-mitigations/(preview|apply)|reroute-bundles/.*/revert|notifications/.*/test|devices/.*/(discover|ssh-test))' "$script"; then exit 1; fi
...
echo "recovery progress deployment preservation and recovery checks: ok"
